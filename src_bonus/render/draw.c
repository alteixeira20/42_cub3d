...
/*   Updated: 2025/09/10 19:17:32 by paalexan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_helper(t_game *cube, t_ray *r, int x)
{
	...
	while (y < r->draw_start)
		put_pixel_img(&cube->render.frame, x, y++, cube->ceil_color.argb);
	while (y <= r->draw_end)
	{
		tex_y = (int)r->tex_pos;
		if (tex_y < 0)
		...
		put_pixel_img(&cube->render.frame, x, y, color);
		r->tex_pos += r->step;
		y++;
	}
}
...
static void	draw_column(t_game *cube, int x, t_ray *r)
{
	...
	while (y < SCR_H)
	{
		put_pixel_img(&cube->render.frame, x, y, cube->floor_color.argb);
		y++;
	}
}
